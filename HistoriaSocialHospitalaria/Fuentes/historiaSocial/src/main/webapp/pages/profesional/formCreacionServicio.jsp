...
<title>Formulario de alta - Nuevo Servicio</title>
...
<% String user = (String)session.getAttribute("user"); 
	...
	<%} else { %>
		...
		<div id="dialog" style="display: none;">
		  <p>Primero debe registrar un ingreso antes de agregar un servicio</p>
		</div>	
		...
		<div id="pagina">
			<jsp:include page="../../header.jsp">
 			...
 			<div class="buttonReturn"><a href="abmServicios.action" > Volver </a></div>
    		
			<div id="tabs" style="font-size: 14px">
			    <ul>
			        <li><a href="#fragment-1"><span>Carga de Servicios</span></a></li>
			...
			<form	
			<s:if test="oper == 'edit'">	    
				 action="modificarServicio.action"
			</s:if>
			<s:elseif test="oper == 'create'">
				action="salvarServicio.action"
			</s:elseif>
			<s:else>
				action="salvarServicio.action"
			</s:else>
			class="leftLabel" id="practica" method="post" name="Carga de Practica" target="_parent">
			    <div id="fragment-1">
				<div>
				<s:textfield name="nombreServicio" key="servicio.descripcion" label="Nombre del Servicio" size="50" value="%{servicio.nombreServicio}"/>
				<s:if test="oper == 'edit'">	    
					<input name="servicio.id" type="hidden" value="<s:property value="servicio.id"/>"/>
				</s:if>			
			    ...
			    <s:if test="oper == 'edit' || oper == 'create' || oper == ''">
					...
					<input name="submit" type="submit" value="Aceptar" />&nbsp; <input name="Cancelar" type="button" value="Cancelar" onclick="$(location).attr('href','abmServicios.action'); return false;"/>
					</p>
				</s:if>
			</form>
