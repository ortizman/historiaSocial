	...
	<%} else { %>
		...
		<div id="pagina">
			<jsp:include page="../../header.jsp">
 			...
 			<div class="buttonReturn"><a href="abmTipoPropiedad.action" > Volver </a></div>
    		
			<div id="tabs" style="font-size: 14px">
			    <ul>
			        <li><a href="#fragment-1"><span>Creaci&oacute;n de un Tipo de Propiedad</span></a></li>
			...
			<form	
			<s:if test="oper == 'edit'">	    
				 action="modificarTipoPropiedad.action"
			</s:if>
			<s:elseif test="oper == 'create'">
				action="salvarTipoPropiedad.action"
			</s:elseif>
			<s:else>
				action="salvarTipoPropiedad.action"
			</s:else>
			class="leftLabel" id="creacion_tipoPropiedad" method="post" name="Nuevo Tipo de Propiedad" target="_parent">
			    <div id="fragment-1">
				<div>
					<s:textfield name="tipoDePropiedad" key="tipoDePropiedad" label="Tipo de Propiedad" size="50" value="%{tipoPropiedad.tipoDePropiedad}"/>
			    ...
			    <s:if test="oper == 'edit' || oper == 'create' || oper == ''">
					...
					<input name="submit" type="submit" value="Aceptar" />&nbsp; <input name="Cancelar" type="button" value="Cancelar" onclick="$(location).attr('href','abmTipoPropiedad.action'); return false;" />
					</p>
				</s:if>
			</form>
