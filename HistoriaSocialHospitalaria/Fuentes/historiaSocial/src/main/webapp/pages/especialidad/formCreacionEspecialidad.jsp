...
<title>Formulario de alta - Nueva Especialidad</title>
...
<% String user = (String)session.getAttribute("user"); 
	...
	<%} else { %>
		...
		<div id="dialog" style="display: none;">
		  <p>Primero debe registrar un ingreso antes de agregar una especialidad</p>
		</div>	
		...
		<div id="pagina">
			<jsp:include page="../../header.jsp">
 			...
 			<div class="buttonReturn"><a href="abmEspecialidades.action" > Volver </a></div>
    		
			<div id="tabs" style="font-size: 14px">
			    <ul>
			        <li><a href="#fragment-1"><span>Carga de Especialidades</span></a></li>
			...
			<form	
			<s:if test="oper == 'edit'">	    
				 action="modificarEspecialidad.action"
			</s:if>
			<s:elseif test="oper == 'create'">
				action="salvarEspecialidad.action"
			</s:elseif>
			<s:else>
				action="salvarEspecialidad.action"
			</s:else>
			class="leftLabel" id="especialidad" method="post" name="Carga de Especialidades" target="_parent">
			    <div id="fragment-1">
				<div>
					<s:textfield name="descripcion" key="especialidad.especialidad" label="Especialidad" size="50" value="%{especialidad.descripcion}"/>
			    ...
			    <s:if test="oper == 'edit' || oper == 'create' || oper == ''">
					...
					<input name="submit" type="submit" value="Aceptar" />&nbsp; <input name="Cancelar" type="button" value="Cancelar" onclick="$(location).attr('href','abmEspecialidades.action'); return false;" />
					</p>
				</s:if>
			</form>
